...
Sensor::Sensor() : runned(false), aborted(false)
{
    connect(&socket, SIGNAL(readyRead()), SLOT(OnRead()));
    connect(&socket, SIGNAL(connected()), SIGNAL(OnConnected()));
    connect(&socket, SIGNAL(disconnected()), SIGNAL(OnDisconnected()));
    connect(&socket, SIGNAL(error(QAbstractSocket::SocketError)), SIGNAL(OnError()));
    connect(this, SIGNAL(OnSending(QByteArray)), SLOT(OnSend(QByteArray)));
}
...
void Sensor::Push(const QByteArray& array)
{
    ...
    {
        runned = true;
        QtConcurrent::run(this, &Sensor::Send);
    }
}
...
void Sensor::Send()
{
    forever
    {
        ...
        QByteArray array = queue.front();
        queue.pop_front();
        locker.unlock();
        for(int i = 0; i < 5; ++i)
        {
            emit OnSending(array);
            QThread::msleep(100);
        }
    }
}
...
void Sensor::Parse()
{
    if(array.size() < (int)sizeof(DataHeader)) return;
    DataHeader header = *reinterpret_cast<DataHeader*>(array.data());
    Converter::Convert(header);
    if(header.MagicWord != 0xAFFEC0C2) return;

    if(header.DataType == 0x2202)
    {
        if(array.size() < (int)(sizeof(DataHeader) + sizeof(ScanHeader))) return;
        ScanHeader scanHeader = *reinterpret_cast<ScanHeader*>(array.data() + sizeof(DataHeader));
        if(array.size() != (int)(sizeof(DataHeader) + sizeof(ScanHeader) +
        ...
        {
            ScanPoint scanPoint = *reinterpret_cast<ScanPoint*>(array.data() +
    ...
    {
        if(array.size() != sizeof(DataHeader) + sizeof(ErrorRegisters)) return;
        ErrorRegisters regs = *reinterpret_cast<ErrorRegisters*>(array.data() +
            sizeof(DataHeader));
    ...
    {
        if(array.size() < (int)(sizeof(DataHeader) + sizeof(quint16))) return;
        Reply replyId = *reinterpret_cast<Reply*>(array.data() + sizeof(DataHeader));
        ...
        {
            if(array.size() != sizeof(DataHeader) + sizeof(quint16) + sizeof(ReplyStatus)) return;
            ReplyStatus param = *reinterpret_cast<ReplyStatus*>(array.data() +
        ...
        {
            if(array.size() != sizeof(DataHeader) + sizeof(quint16) + sizeof(ReplyGetParameter))
                return;
            ReplyGetParameter param = *reinterpret_cast<ReplyGetParameter*>(array.data()
                + sizeof(DataHeader) + sizeof(quint16));
            Converter::Convert(param);
            Parameters p;
            ...
            if(param.ParameterIndex == 0x1105)
            {
                p.AngleTicksPerRotation = param.Parameter;
                p.ParameterChanged = Parameter::AngleTicksPerRotation;
            }
...
void Sensor::OnSend(const QByteArray& array)
{
    socket.write(array);
}
...
void Sensor::OnRead()
{
    ...
    if(a.size() < (int)(sizeof(quint32))) return;
    DataHeader header;
    header.MagicWord = *reinterpret_cast<quint32*>(a.data());
    Converter::Convert(header);
    Parse();
    if(header.MagicWord == 0xAFFEC0C2) array.clear();
    array.append(a);
}
